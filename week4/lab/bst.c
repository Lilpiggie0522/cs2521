...
int bstClosest(struct node *t, int val) {
	...
	if (t->left == NULL) {
		int right_close = bstClosest(t->right, val);
		int right_diff = abs(right_close - val);
		int curr_diff = abs(t->value - val);
		if (right_diff < curr_diff) {
			return t->right->value;
		}
		return t->value;
	} else if (t->right == NULL) {
		int left_close = bstClosest(t->left, val);
		int left_diff = abs(left_close - val);
		int curr_diff = abs(t->value - val);
...
void bstLevelOrder(struct node *t) {
	...
	Queue new_q = QueueNew();
	QueueEnqueue(new_q, t);
	while (!QueueIsEmpty(new_q)) {
		struct node *p = (struct node *)QueueDequeue(new_q);
		if (p != NULL) {
			printf("%d ", p->value);
			QueueEnqueue(new_q, p->left);
			QueueEnqueue(new_q, p->right);
		}
	}
	free(new_q);
}
