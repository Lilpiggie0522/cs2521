...
void HashTableInsert(HashTable table, int key, int value) {
	...
	int rightSpot = hash(key, table->numSlots);

	for (int i = rightSpot; i < table->numSlots; i ++) {
		if (table->slots[i].empty == true || table->slots[i].key == key) {
			rightSpot = i;
			break;
		}	
	}

	if (table->slots[rightSpot].empty == true) {
		table->slots[rightSpot].key = key;
		table->slots[rightSpot].value = value;
		table->slots[rightSpot].empty = false;
		table->numItems ++;
		return;
	}
	table->slots[rightSpot].value = value;
...
void HashTableDelete(HashTable table, int key) {
	...
	for (int i = index; i < table->numSlots; i ++) {
	...
	//	deletion?
	table->slots[correctIndex].empty = true;

	// re-insert everything after this
	for (int i = correctIndex + 1; i < table->numSlots; i ++) {
		if (table->slots[i].empty != true) {
			table->slots[i].empty = true;
			HashTableInsert(table, table->slots[i].key, table->slots[i].value);
		}
	}
}
