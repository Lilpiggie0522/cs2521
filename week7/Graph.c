...
static struct adjNode *doInsertEdge(struct adjNode *list, Vertex v, int weight);
...
void GraphInsertEdge(Graph g, Vertex v, Vertex w, int weight) {
	...
	if (!GraphIsAdjacent(g, v, w)) {
		g->edges[v] = doInsertEdge(g->edges[v], w, weight);
		g->nE++;
	}
}
...
static struct adjNode *doInsertEdge(struct adjNode *list, Vertex v, int weight) {
	if (list == NULL || v < list->v) {
		...
		list->next = doInsertEdge(list->next, v, weight);
		return list;
	}
}
...
Graph GraphRead(FILE *in) {
	...
	int w = 0;
	int weight = 0;
	while (fscanf(in, "%d %d %d", &v, &w, &weight) == 3) {
