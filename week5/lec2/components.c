...
static int *components(Graph g) {
	...
	for (Vertex v = 0; v < g->nV; v ++) {
		if (componentOf[v] == -1) {
			dfsComponents(g, v, componentOf, componentNo);
			componentNo ++;
		}
	}
	return componentOf;
...
static void dfsComponents(Graph g, Vertex v, int *componentOf, int componentNo) {
	
	componentOf[v] = componentNo;	// this line has to be here, otherwise when all vertecies are not connected, no increments is performed
	struct adjNode *curr = g->edges[v];
	for (; curr != NULL; curr = curr->next) {
		Vertex w = curr->v;
		if (componentOf[w] == -1) {
			dfsComponents(g, w, componentOf, componentNo); // Pass in w not v!
		}
	}
}
